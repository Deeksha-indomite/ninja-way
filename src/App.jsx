...
async function signOut() {
  ...
  const TABS=[{id:"missions",icon:"🗡️",label:"MISSIONS"},{id:"battle",icon:"⚔️",label:"BATTLE"},{id:"exam",icon:"📋",label:"STUDY"},{id:"stats",icon:"📊",label:"STATS"},{id:"about",icon:"🐦‍🔥",label:"ABOUT"},{id:"profile",icon:"🥷",label:"PROFILE"}];
    ...
    <motion.button onClick={signOut} whileTap={{scale:.95}}
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:"rgba(13,5,0,.96)",borderTop:"1px solid rgba(255,200,100,.1)",backdropFilter:"blur(12px)",zIndex:1000,padding:"7px 0 8px"}}>
        <div style={{display:"flex",maxWidth:460,margin:"0 auto"}}>
          {TABS.map(t=>(
            <motion.button key={t.id} onClick={()=>t.id==="about"?navigate("/about"):t.id==="profile"?navigate("/profile"):setTab(t.id)} whileTap={{scale:.9}} style={{flex:1,padding:"7px 4px",background:"transparent",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,outline:"none"}}>
